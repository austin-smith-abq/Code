...
CASE
    ...
    --     ) THEN 'Launched;Pending'
    -- WHEN (
    --     ChargeDispo.ChargeDispoDesc IS NULL AND launched_status.case_stage = 'Warrant Outstanding'
    --     ) THEN 'Warrant Outstanding;Pending'  
    -- WHEN (
    --     ChargeDispo.ChargeDispoDesc IS NULL AND launched_status.case_stage = 'Competency Evaluation'
    --     ) THEN 'Competency Evaluation;Pending' 
...
CASE WHEN(
    SUM(CASE
        WHEN
        charge_level_summary.charge_trajectory = 'Warrant Outstanding' THEN 1 ELSE 0 END) OVER (PARTITION BY charge_level_summary.CrimeCase) > 0)
    THEN 'Warrant Outstanding'
 WHEN(
    SUM(CASE
        WHEN
        charge_level_summary.charge_trajectory = 'Competency Evaluation' THEN 1 ELSE 0 END) OVER (PARTITION BY charge_level_summary.CrimeCase) > 0)
    THEN 'Competency Evaluation'
 WHEN(
    SUM(CASE
        WHEN
 ...
 WHEN(
    SUM(CASE
        WHEN
        charge_level_summary.charge_trajectory = 'Launched' THEN 1 ELSE 0 END) OVER (PARTITION BY charge_level_summary.CrimeCase) > 0)
    THEN 'Launched'
 WHEN(
    SUM(CASE
        WHEN
