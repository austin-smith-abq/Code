...
--
    FROM CrimeCase
INNER JOIN CaseClass ON CrimeCase.CaseClass = CaseClass.CaseClass
INNER JOIN CaseStage ON CrimeCase.CaseStage = CaseStage.CaseStage
INNER JOIN CaseStatus ON CaseStage.CaseStatus = CaseStatus.CaseStatus
INNER JOIN CaseType as CaseType ON CrimeCase.CaseType = CaseType.CaseType
INNER JOIN CrimeDivision as CrimeDivision ON CrimeCase.CrimeDivision = CrimeDivision.CrimeDivision
--
INNER JOIN Person as Defendant on CrimeCase.Defendant = Defendant.Person
INNER JOIN Race as DefendantRace on Defendant.Race = DefendantRace.Race
INNER JOIN Ethnicity as DefendantEthnicity on Defendant.Ethnicity = DefendantEthnicity.Ethnicity
INNER JOIN Alias as DefendantAlias on Defendant.Person = DefendantAlias.Person
-- 
INNER JOIN Alias as ProsecutorAlias on CrimeCase.Prosecutor = ProsecutorAlias.Person
-- 
INNER JOIN Alias as DefenseAlias on CrimeCase.DefenseAtty = DefenseAlias.Person
--
INNER JOIN Alias as AgentAlias on CrimeCase.Agent = AgentAlias.Person
--
INNER JOIN Alias as ScreenerAlias on CrimeCase.Screener = ScreenerAlias.Person
--
INNER JOIN Alias as InvestigatorAlias on CrimeCase.Investigator = InvestigatorAlias.Person
--
INNER JOIN Alias as VictimAdvocateAlias on CrimeCase.VicAdv = VictimAdvocateAlias.Person
-- 
INNER JOIN Alias as DistrictJudgeAlias on CrimeCase.DistJudge = DistrictJudgeAlias.Person
-- 
INNER JOIN Alias as MagistrateJudgeAlias on CrimeCase.MagJudge = MagistrateJudgeAlias.Person
--
INNER JOIN Agency as Agency on CrimeCase.Agency = Agency.Agency
WHERE 1 = 1 
...
FROM case_list
INNER JOIN Charges on Charges.CrimeCase = case_list.CrimeCase
INNER JOIN ChargeCode on ChargeCode.ChargeCode = Charges.ChargeCode
),
...
--
INNER JOIN Charges on launched_status.CrimeCase = Charges.CrimeCase
INNER JOIN ChargeCode on ChargeCode.ChargeCode = Charges.ChargeCode
INNER JOIN ChargeDispo on Charges.PleaDispo = ChargeDispo.ChargeDispo
INNER JOIN Reason on Charges.Reason = Reason.Reason
INNER JOIN ChargeMethod as ChargeMethod on Charges.ChargeMethod = ChargeMethod.ChargeMethod
INNER JOIN Degree on ChargeCode.Degree = Degree.DegreeCode
WHERE 2 = 2
